...
public class DragonVortexRetail extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		if (event.equalsIgnoreCase("Spawn"))
		{
			...
			if (random < 145)
			{
				raid = RAIDS[0]; // Emerald Horn 14.5%
			}
			else if (random < 290)
			{
				raid = RAIDS[1]; // Dust Rider 14.5%
			}
			else if (random < 435)
			{
				raid = RAIDS[2]; // Bleeding Fly 14.5%
			}
			else if (random < 580)
			{
				raid = RAIDS[3]; // Blackdagger Wing 14.5%
			}
			else if (random < 725)
			{
				raid = RAIDS[4]; // Spike Slasher 14.5%
			}
			else if (random < 870)
			{
				raid = RAIDS[5]; // Shadow Summoner 14.5%
			}
			else
			{
				raid = RAIDS[6]; // Muscle Bomber 13.0%
			}
